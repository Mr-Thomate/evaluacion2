...
public class EditorialService {
    ...
    public EditorialDTO buscarPorId(Integer id) {
        return editorialRepository.findById(id)
                .map(this::convertirADTO)
                .orElseThrow(() -> new RuntimeException("Error: No se encontró la editorial con el ID: " + id));
    }
    ...
    private EditorialDTO convertirADTO(Editorial editorial) {
        ...
        if (editorial.getLibroEditorial() != null) {
            dto.setTituloLibros(editorial.getLibroEditorial().stream()
                .map(le -> le.getLibro().getTitulo())
                .collect(Collectors.toList()));
        }
        return dto;
    }
